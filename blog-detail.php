...
// Ensure we have a blog ID in the URL
...
// Fetch blog post and associated images (if any)
$blog_query = "SELECT b.*, i.image_path, i.is_featured 
               FROM blogs b 
               LEFT JOIN blog_images i ON b.id = i.blog_id 
               WHERE b.id = '$blog_id'";
...
// Loop through the results to collect images and blog details
while ($row = mysqli_fetch_assoc($blog_result)) {
    if (!$blog) {
        $blog = $row; // Set blog details only once
    }
    ...
    if (!empty($row['image_path'])) {
        ...
        if ($row['is_featured'] == 1) {
            $featured_image = $row['image_path'];
        }
    }
}

// If no featured image is found, use the first one (if available)
if (empty($featured_image) && count($images) > 0) {
    $featured_image = $images[0]['image_path'];
}

// Format the date
$post_date = date('M d, Y', strtotime($blog['created_at']));
?>
...
<head>
    ...
    <title><?php echo htmlspecialchars($blog['title']); ?> - Odex</title>
                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <div class="text-center">
                            <h2 class="breadcrumbs_title"><?php echo htmlspecialchars($blog['title']); ?></h2>
                        ...
                        <article class="blog-news big-detail-wrap">
                            <div class="blog-detail-wrap">
                                ...
                                <figure class="img-holder">
                                    <img src="admin/<?php echo htmlspecialchars($featured_image); ?>"
                                        class="img-responsive" alt="<?php echo htmlspecialchars($blog['title']); ?>">
                                ...
                                <div class="full blog-content">
                                    <div class="post-meta">By: <a href="#"
                                            class="author theme-cl"><?php echo htmlspecialchars($blog['author']); ?></a>
                                        <?php if(isset($blog['comment_count'])): ?>
                                        | <?php echo $blog['comment_count']; ?>
                                        comment<?php echo $blog['comment_count'] != 1 ? 's' : ''; ?>
                                        <?php endif; ?>
                                    </div>
                                    <a href="blog-detail.php?id=<?php echo $blog_id; ?>">
                                    ...
                                    <div class="blog-text">
                                        ...
                                        <div class="post-meta">Filed Under: <span class="category"><a
                                                    href="blog.php?category=<?php echo urlencode($blog['category']); ?>"
                                                    class="theme-cl"><?php echo htmlspecialchars($blog['category']); ?></a></span>
                                        </div>
                                        <?php endif; ?>
                                    </div>

                                    <!-- Additional Images Gallery (if needed) -->
                                    <?php if (count($images) > 1): ?>
                                    <div class="blog-gallery mt-4">
                                        <div class="row">
                                            <?php foreach($images as $img): ?>
                                            <?php if($img['image_path'] != $featured_image): ?>
                                            <div class="col-md-4 mb-3">
                                                <a href="admin/<?php echo htmlspecialchars($img['image_path']); ?>"
                                                    target="_blank">
                                                    <img src="admin/<?php echo htmlspecialchars($img['image_path']); ?>"
